...
#Docker must be installed before run script.
# User must be in Docker group
# By positional parametr $1 we get the user name to create cluster and config deliberately to him.
# Author: localsnet
...
# For AMD64 / x86_64
if [[ $arch = x86_64 ]]; then
sudo -u $1  curl -Lo ./kind https://kind.sigs.k8s.io/dl/v$version/kind-linux-amd64
# For ARM64
elif [[ $arch = aarch64 ]]; then
 sudo -u $1 curl -Lo ./kind https://kind.sigs.k8s.io/dl/v$version/kind-linux-arm64
else
...
sudo -u $1 kind create cluster --name $cluster_name --config cluster_3_nodes.yaml